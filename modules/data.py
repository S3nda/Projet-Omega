...
def pres_configs(configs):
    """Présente les configurations de chaque partie"""
...
def search_nom(nom):
    """Renvoie un dictionnaire contenant les stats d'un joueur"""
...
def crea_dico_dans_csv(dico_stats, fichier="stats.csv"):
    """Crée un fichier csv avec les stats d'un joueur"""
    ...
    path_fichier = os.path.join(os.path.abspath(os.path.dirname(__file__), ), "Data", fichier)
    ...
    else:  # si le joueur n'existe pas, on crée un nouveau fichier
...
def pres_stats_joueur(stats):
    """Présente-les stats d'un joueur"""
    data = {
        'Nom': [stats['nom']],
    ...
    for key, value in data.items():  # on affiche les stats, en les alignant correctement
...
def stats_casino():
    """Renvoie un dictionnaire contenant les stats du casino"""
    path_fichier = os.path.join(os.path.abspath(os.path.dirname(__file__)), "Data", "stats.csv")
    try:
        dataframe = pd.read_csv(path_fichier)
...
def pres_stats_casino(stats):
    ...
    data = {
        ...
        'Bénéfices': [stats["Gains_totaux"] - stats['Chiffre d\'affaire'], "€"]
    }
...
def default():
    """Renvoie un dictionnaire contenant les stats par défaut"""
    return {
        'nom': None,
        'num_entree': 0,
        'nbr_tours_total': 0,
